...
export default async function AdminLayout({ children }: { children: ReactNode }) {
  ...
  if (!isAdminEmail(user.email)) {
    return <AdminAccessDenied email={user.email ?? null} />;
  }
...
function AdminAccessDenied({ email }: { email: string | null }) {
  return (
    <div dir="rtl" className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white p-6 text-right shadow-sm">
        ...
        <h1 className="text-xl font-semibold text-gray-900">صلاحية الأدمن مطلوبة</h1>
        <p className="mt-2 text-sm leading-6 text-gray-600">
          هذا الحساب مسجل دخوله، لكنه غير مضاف إلى قائمة مديري النظام.
        </p>
        {email ? (
          <p className="mt-3 rounded-lg bg-gray-50 px-3 py-2 text-xs text-gray-500">
            البريد الحالي: <span className="font-medium text-gray-700">{email}</span>
          </p>
        ) : null}
        <p className="mt-3 text-xs leading-6 text-gray-500">
          أضف البريد إلى متغير البيئة <code className="rounded bg-gray-100 px-1 py-0.5">ADMIN_EMAILS</code> في Vercel ثم أعد النشر.
        </p>
        <Link
          href="/"
